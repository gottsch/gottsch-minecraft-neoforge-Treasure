...
public enum RarityTagAssociationRegistry {
    ...
    public static Optional<TagKey<Block>> getChestTagKey(IRarity rarity, HolderLookup.Provider registries) {
    ...
    public static List<Block> getChestBlocks(IRarity rarity, HolderLookup.Provider registries) {
//
...
//        List<Block> blocks = tag.stream().toList();
        ...
        return getChestTagKey(rarity, registries)
                .map(tagKey -> {
                    var a = BuiltInRegistries.BLOCK.getOrCreateTag(tagKey)
                            .stream()
        ...
        if (blockHolderOptional.isPresent()) {
            ...
            for (RarityTagAssociation association : associations) {
                ...
                if (blockHolder.is(tagKey)) {
                    ...
                    return TreasureRarities.getRarityByName(association.rarityId(), registries);
                }
            }
        }
    ...
    public static Optional<TagKey<Item>> getKeysTagKey(IRarity rarity, HolderLookup.Provider registries) {
    ...
    public static List<Item> getKeyItems(IRarity rarity, HolderLookup.Provider registries) {

//        Optional<TagKey<Item>> keysTagKey = getKeysTagKey(rarity, registries);
//        List<Item> items = BuiltInRegistries.ITEM.getOrCreateTag(keysTagKey.get())
//                .stream().map(Holder::value).toList();

        return getKeysTagKey(rarity, registries)
                .map(tagKey -> BuiltInRegistries.ITEM.getOrCreateTag(tagKey)
                        .stream().map(Holder::value).toList())
                .orElse(Collections.emptyList());
    }
    ...
    public static Optional<TagKey<Item>> getLocksTagKey(IRarity rarity, HolderLookup.Provider registries) {
    ...
    public static List<Item> getLockItems(IRarity rarity, HolderLookup.Provider registries) {
...
//        List<Item> items = tag.stream().toList();
        ...
        return getLocksTagKey(rarity, registries)
                .map(tagKey -> BuiltInRegistries.ITEM.getOrCreateTag(tagKey)
                        .stream().map(Holder::value).toList())
                .orElse(Collections.emptyList());
    }
