 ...
 */
package mod.gottsch.neoforge.treasure2.core.cache.data;
...
import mod.gottsch.neoforge.treasure2.core.world.feature.FeatureType;
...
// TODO needs to be converted to an Attachment
/**
...
public class TreasureChestCacheData {
    ...
    public CompoundTag save(CompoundTag tag, HolderLookup.Provider provider) {
        ...
        tag.putString(RARITY_NAME, TreasureRarities.getKey(getRarity(), provider).map(ResourceLocation::toString).orElse(TreasureRarities.COMMON.getId().toString()));
    ...
    public void load(CompoundTag tag) {
        if (tag.contains(CHEST_NAME)) {
            this.chestName = ModUtil.asLocation(tag.getString(CHEST_NAME)).orElse(null);
        }

        if (tag.contains(DIMENSION_NAME)) {
            this.dimensionName = ModUtil.asLocation(tag.getString(DIMENSION_NAME)).orElseGet(() -> ResourceLocation.fromNamespaceAndPath("minecraft", "overworld"));
        }

        if (tag.contains(BIOME_NAME)) {
            this.biomeName = ModUtil.asLocation(tag.getString(BIOME_NAME)).orElse(null);
        }

//        if (tag.contains(FEATURE_TYPE)) {
//            // TODO get from Feature registry
//            TreasureFeatureTypes.getFeatureTypeByName()
//            this.featureType = TreasureApi.getFeatureType(tag.getString(FEATURE_TYPE).toUpperCase()).orElse(TreasureFeatureTypes.UNKNOWN);
//        }
        this.featureType = Optional.ofNullable(tag.getString(FEATURE_TYPE))
                .flatMap(ModUtil::asLocation)
                .flatMap(TreasureFeatureTypes::getFeatureTypeByName)
                .orElse(TreasureFeatureTypes.UNKNOWN.get());
...
//        }
        ...
        this.rarity = Optional.ofNullable(tag.getString(RARITY_NAME))
                .flatMap(ModUtil::asLocation)
                .flatMap(TreasureRarities::getRarityByName)
                .orElse(TreasureRarities.UNKNOWN.get());


//        if (tag.contains(LOOT_TABLE_NAME)) {
//            this.lootTableName = ModUtil.asLocation(tag.getString(LOOT_TABLE_NAME));
//        }
        this.lootTableName = Optional.ofNullable(tag.getString(LOOT_TABLE_NAME))
                .flatMap(ModUtil::asLocation)
                .orElse(null);
