...
/**
 * The bone lock is a "permanent" lock on a Bone Chest: it is never removed from the
 * lock state and is never dropped as an item.
 *
...
public class BoneLock extends LockItem {
	...
	public void doUnlock(Level level, Player player, BlockPos chestPos, LockState lockState) {
		...
		// update the chest's locked property (this can also be executed from the BoneKey)
