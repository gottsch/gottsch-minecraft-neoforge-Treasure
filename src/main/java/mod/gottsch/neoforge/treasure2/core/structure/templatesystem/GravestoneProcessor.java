...
import mod.gottsch.neoforge.treasure2.core.block.GravestoneBlock;
...
/**
 * Randomly replaces gravestone marker blocks with gravestone / gravestone-spawner variants,
 * relocating them to a valid ground position, and finalizes two-part skeletons.
 *
...
public class GravestoneProcessor extends StructureProcessor {
    ...
    // spawner-variant gravestones (the port has no TreasureBlocks.GRAVESTONE_SPAWNERS collection)
    public static final List<Block> GRAVESTONE_SPAWNERS = List.of(
    ...
    public static final MapCodec<GravestoneProcessor> CODEC = MapCodec.unit(GravestoneProcessor::new);
    ...
    public StructureTemplate.StructureBlockInfo processBlock(LevelReader levelReader, BlockPos pos, BlockPos relativePos, StructureTemplate.StructureBlockInfo original, StructureTemplate.StructureBlockInfo current, StructurePlaceSettings placementSettings) {
        ...
        if (Config.SERVER.markers.enableSpawner.get() &&
            ...
            newState = GRAVESTONE_SPAWNERS.get(random.nextInt(GRAVESTONE_SPAWNERS.size())).defaultBlockState();
        } else {
            // select a gravestone
    ...
    public List<StructureTemplate.StructureBlockInfo> finalizeProcessing(ServerLevelAccessor level, BlockPos spawnPos, BlockPos maxPos, List<StructureTemplate.StructureBlockInfo> originalBlocks, List<StructureTemplate.StructureBlockInfo> processedBlocks, StructurePlaceSettings placeSettings) {
        ...
        // return a new list that doesn't contain the removed placeholders
