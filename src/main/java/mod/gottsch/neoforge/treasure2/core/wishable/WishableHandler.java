...
public class WishableHandler implements IWishableHandler {
    ...
    public Optional<ItemStack> generateLoot(Level level, Random random, ItemEntity itemEntity, ICoords coords) {
        ...
        IRarity rarity = TreasureWishables.getRarity(itemEntity.getItem().getItem(), level.registryAccess())
        ...
        if (lootTableNames.isEmpty()) {
            return Optional.of(getDefaultLootKey(random, rarity, level.registryAccess()));
        }
     ...
     * @return
     */
    public ItemStack getDefaultLootKey (Random random, ItemEntity itemEntity, HolderLookup.Provider registries) {
        IRarity rarity = TreasureWishables.getRarity(itemEntity.getItem().getItem(), registries).orElse(TreasureRarities.COMMON.get());
        return getDefaultLootKey(random, rarity, registries);
    }
     ...
     * @return
     */
    public ItemStack getDefaultLootKey(Random random, IRarity rarity, HolderLookup.Provider registries) {
        List<Item> keys = RarityTagAssociationRegistry.getKeyItems(rarity, registries);
