...
public class MythicalChestSubprocessor extends HighTierRarityChestSubprocessor {
    ...
    @Override
    public List<AbstractTreasureChestBlock> getValidChests(final IRarity rarity, final HolderLookup.Provider provider) {

        List<AbstractTreasureChestBlock> validChests = super.getValidChests(rarity, provider);

        // add all using a lesser rarity (legendary)
        IRarity rarityMinusOne = ChestGenerationHelper.getBoostedRarity(provider, rarity, -1).orElse(rarity);
        validChests.addAll(super.getValidChests(rarityMinusOne, provider));

        // add all using an even lesser rarity (epic)
        IRarity rarityMinusTwo = ChestGenerationHelper.getBoostedRarity(provider, rarity, -2).orElse(rarity);
        validChests.addAll(super.getValidChests(rarityMinusTwo, provider));
