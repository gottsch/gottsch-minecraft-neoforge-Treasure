...
public class TreasureWishables {
	 ...
	 */
	public static Optional<IRarity> getRarity(Item item, HolderLookup.Provider lookupProvider) {
		return getRarityAssociations().stream()
				.filter(association -> item.builtInRegistryHolder().is(association.getItemTag()))
				.findFirst()
				.flatMap(association -> TreasureRarities.getRarityByName(association.rarityId(), lookupProvider));
	}
